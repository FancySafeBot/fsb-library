...
TEST_CASE("Interface forward kinematics" * doctest::description("[fsb_compute_kinematics][fsb::BodyTree::add_body]"))
{
    ...
    const fsb::Vec3 expected_vel_lin = {0.37144459633699844, 0.018381861441678617, 0.5513884571954082};
    const fsb::Vec3 expected_acc_ang = {2.413285107399128, 1.5184808448401204, -1.5152061882313153};
    const fsb::Vec3 expected_acc_lin = {-2.0026812338083984, 2.0395778544442376, -1.1720634682396098};
    ...
    // Expected
    const fsb::CartesianPva expected_body3_pva = {
    ...
    compute.compute_forward_kinematics_with_base(opt, joint_pva, base_pva, actual_body_pva);

    // Check
    const fsb::CartesianPva &body3_pva =  actual_body_pva.body[body3_index];

    REQUIRE(expected_body3_pva.pose.translation.x == FsbApprox(body3_pva.pose.translation.x));
    REQUIRE(expected_body3_pva.pose.translation.y == FsbApprox(body3_pva.pose.translation.y));
    REQUIRE(expected_body3_pva.pose.translation.z == FsbApprox(body3_pva.pose.translation.z));

    REQUIRE(1.0 == FsbApprox(fsb::quat_norm(body3_pva.pose.rotation)));
    REQUIRE(expected_body3_pva.pose.rotation.qw == FsbApprox(body3_pva.pose.rotation.qw));
    REQUIRE(expected_body3_pva.pose.rotation.qx == FsbApprox(body3_pva.pose.rotation.qx));
    REQUIRE(expected_body3_pva.pose.rotation.qy == FsbApprox(body3_pva.pose.rotation.qy));
    REQUIRE(expected_body3_pva.pose.rotation.qz == FsbApprox(body3_pva.pose.rotation.qz));

    REQUIRE(expected_body3_pva.velocity.linear.x == FsbApprox(body3_pva.velocity.linear.x));
    REQUIRE(expected_body3_pva.velocity.linear.y == FsbApprox(body3_pva.velocity.linear.y));
    REQUIRE(expected_body3_pva.velocity.linear.z == FsbApprox(body3_pva.velocity.linear.z));

    REQUIRE(expected_body3_pva.velocity.angular.x == FsbApprox(body3_pva.velocity.angular.x));
    REQUIRE(expected_body3_pva.velocity.angular.y == FsbApprox(body3_pva.velocity.angular.y));
    REQUIRE(expected_body3_pva.velocity.angular.z == FsbApprox(body3_pva.velocity.angular.z));

    REQUIRE(expected_body3_pva.acceleration.linear.x == FsbApprox(body3_pva.acceleration.linear.x));
    REQUIRE(expected_body3_pva.acceleration.linear.y == FsbApprox(body3_pva.acceleration.linear.y));
    REQUIRE(expected_body3_pva.acceleration.linear.z == FsbApprox(body3_pva.acceleration.linear.z));

    REQUIRE(expected_body3_pva.acceleration.angular.x == FsbApprox(body3_pva.acceleration.angular.x));
    REQUIRE(expected_body3_pva.acceleration.angular.y == FsbApprox(body3_pva.acceleration.angular.y));
