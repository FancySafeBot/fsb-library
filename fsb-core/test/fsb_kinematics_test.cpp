...
TEST_CASE("Forward Kinematics SRS" * doctest::description("[fsb_kinematics][fsb::forward_kinematics]"))
{
    // Inputs
    const fsb::Vec3 base_position = {0.12, -0.34, 0.921};
    const fsb::Quaternion base_rotation = {0.713252796614972, 0.110018106106709, 0.314124660380793, 0.616840467374075};
    ...
    const fsb::Vec3 joint1_position = {-0.872, 1.235, -0.02};
    const fsb::Quaternion joint1_rotation = {0.57072141808226, 0.575121276132167, 0.0939451898978092, 0.578521289130613};
    const fsb::Quaternion joint1_qpos = { 0.2082929004216056, 0.45810638043384594, -0.6268184243409775, -0.5948539944780146};
    const fsb::Vec3 joint1_qvel = {-0.51, 0.003, 0.443};
    const fsb::Vec3 joint1_qacc = {0.022, -1.23, 0.998};
    ...
    const fsb::Vec3 joint3_position = {0.1, -0.8, 1.4};
    const fsb::Quaternion joint3_rotation = {0.461283965309215, 0.607100248856612, 0.205771002489209, -0.613436782171915};
    const fsb::Quaternion joint3_qpos = {0.44656205339018834, -0.5178471877140327, 0.7161490741756802, 0.13981103749676618};
    const fsb::Vec3 joint3_qvel = {0.0066, -0.1, 0.02};
    const fsb::Vec3 joint3_qacc = {1, 2.1, -0.82};
    ...
    const fsb::JointPva joint_pva = {
        {{joint1_qpos.qw, joint1_qpos.qx, joint1_qpos.qy, joint1_qpos.qz, joint2_qpos, joint3_qpos.qw, joint3_qpos.qx, joint3_qpos.qy, joint3_qpos.qz}},
        {{joint1_qvel.x, joint1_qvel.y, joint1_qvel.z, joint2_qvel, joint3_qvel.x, joint3_qvel.y, joint3_qvel.z}},
        {{joint1_qacc.x, joint1_qacc.y, joint1_qacc.z, joint2_qacc, joint3_qacc.x, joint3_qacc.y, joint3_qacc.z}}
    };

    // Expected
    const fsb::Vec3 expected_position = {0.3174018799132783, -0.030452331246670683, 2.172272625560503};
    const fsb::Quaternion expected_rotation = {0.5627932892016856, 0.21900766283546128, -0.33093749523152394, -0.7251067035263056};
    const fsb::Vec3 expected_vel_ang = {1.2204293093829113, -0.21985522316113404, -0.07967754525441904};
    const fsb::Vec3 expected_vel_lin = {-0.9999385950794331, -0.197661371952973, 1.8727448734327004};
    const fsb::Vec3 expected_acc_ang = {0.3850843097931615, 2.6720383039257953, 0.513010742941556};
    const fsb::Vec3 expected_acc_lin = {0.7434348978894469, -3.414553310154864, -0.20628965530890897};
    ...
    size_t body3_index = 3U;
    constexpr fsb::MassProps unit_mass_props = { 1.0, {}, {1.0, 1.0, 1.0, 0.0, 0.0, 0.0}};
    fsb::BodyTree body_tree = body_tree_sample_srs(
        joint1_tr, joint2_tr, joint3_tr,
        unit_mass_props, unit_mass_props, unit_mass_props,
        body3_index);
    fsb::BodyCartesianPva actual_body_pva = {};
