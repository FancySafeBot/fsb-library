...
Transform joint_parent_child_transform(const Joint& joint, const JointSpacePosition& position)
{
    ...
    {
        result
            = {quat_multiply(tr_pj.rotation, quat_rx(s_neg * position[joint.coord_index])),
               tr_pj.translation};
    }
    else if (joint.type == JointType::REVOLUTE_Y)
    {
        result
            = {quat_multiply(tr_pj.rotation, quat_ry(s_neg * position[joint.coord_index])),
               tr_pj.translation};
    }
    else if (joint.type == JointType::REVOLUTE_Z)
    {
        result
            = {quat_multiply(tr_pj.rotation, quat_rz(s_neg * position[joint.coord_index])),
               tr_pj.translation};
    }
    else if (joint.type == JointType::PRISMATIC_X)
    {
        const Vec3 pos
            = quat_rotate_vector(tr_pj.rotation, {s_neg * position[joint.coord_index], 0.0, 0.0});
        result = {tr_pj.rotation, vector_add(tr_pj.translation, pos)};
    }
    else if (joint.type == JointType::PRISMATIC_Y)
    {
        const Vec3 pos
            = quat_rotate_vector(tr_pj.rotation, {0.0, s_neg * position[joint.coord_index], 0.0});
        result = {tr_pj.rotation, vector_add(tr_pj.translation, pos)};
    }
    else if (joint.type == JointType::PRISMATIC_Z)
    {
        const Vec3 pos
            = quat_rotate_vector(tr_pj.rotation, {0.0, 0.0, s_neg * position[joint.coord_index]});
        result = {tr_pj.rotation, vector_add(tr_pj.translation, pos)};
    }
    else if (joint.type == JointType::SPHERICAL)
    {
        const Quaternion quat_joint
            = {position[joint.coord_index],
               position[joint.coord_index + 1U],
               position[joint.coord_index + 2U],
               position[joint.coord_index + 3U]};
    ...
    else if (joint.type == JointType::CARTESIAN)
    {
        const Transform tr_joint = {
            {position[joint.coord_index],
             ...
             position[joint.coord_index + 2U],
             position[joint.coord_index + 3U]},
            {position[joint.coord_index + 4U],
             position[joint.coord_index + 5U],
             position[joint.coord_index + 6U]}
        };
...
MotionVector joint_parent_child_velocity(const Joint& joint, const JointSpace& velocity)
{
    ...
    {
        vel_result.angular
            = quat_rotate_vector(tr_pj.rotation, {s_neg * velocity[joint.dof_index], 0.0, 0.0});
    }
    if (joint.type == JointType::REVOLUTE_Y)
    {
        vel_result.angular
            = quat_rotate_vector(tr_pj.rotation, {0.0, s_neg * velocity[joint.dof_index], 0.0});
    }
    if (joint.type == JointType::REVOLUTE_Z)
    {
        vel_result.angular
            = quat_rotate_vector(tr_pj.rotation, {0.0, 0.0, s_neg * velocity[joint.dof_index]});
    }
    else if (joint.type == JointType::PRISMATIC_X)
    {
        vel_result.linear
    ...
    else if (joint.type == JointType::SPHERICAL)
    {
        const Vec3 angular_vel
            = {velocity[joint.dof_index],
               velocity[joint.dof_index + 1U],
               velocity[joint.dof_index + 2U]};
    ...
    else if (joint.type == JointType::CARTESIAN)
    {
        const MotionVector joint_vel = {
            {velocity[joint.dof_index],
             velocity[joint.dof_index + 1U],
             velocity[joint.dof_index + 2U]},
            {velocity[joint.dof_index + 3U],
             velocity[joint.dof_index + 4U],
             velocity[joint.dof_index + 5U]}
        };
        vel_result
...
MotionVector joint_parent_child_acceleration(const Joint& joint, const JointPva& joint_pva)
{
    ...
    if (joint.type == JointType::REVOLUTE_X)
    {
        acceleration.angular = quat_rotate_vector(
            tr_pj.rotation, {s_neg * joint_pva.acceleration[joint.dof_index], 0.0, 0.0});
    }
    if (joint.type == JointType::REVOLUTE_Y)
    {
        acceleration.angular = quat_rotate_vector(
            tr_pj.rotation, {0.0, s_neg * joint_pva.acceleration[joint.dof_index], 0.0});
    }
    if (joint.type == JointType::REVOLUTE_Z)
    {
        acceleration.angular = quat_rotate_vector(
            tr_pj.rotation, {0.0, 0.0, s_neg * joint_pva.acceleration[joint.dof_index]});
    }
    else if (joint.type == JointType::PRISMATIC_X)
    {
        acceleration.linear = quat_rotate_vector(
            tr_pj.rotation, {s_neg * joint_pva.acceleration[joint.dof_index], 0.0, 0.0});
    }
    else if (joint.type == JointType::PRISMATIC_Y)
    {
    ...
    else if (joint.type == JointType::SPHERICAL)
    {
        const Vec3 angular_acc
            = {joint_pva.acceleration[joint.dof_index],
               joint_pva.acceleration[joint.dof_index + 1U],
               joint_pva.acceleration[joint.dof_index + 2U]};
    ...
    else if (joint.type == JointType::CARTESIAN)
    {
        const MotionVector joint_acc = {
            {joint_pva.acceleration[joint.dof_index],
             joint_pva.acceleration[joint.dof_index + 1U],
             joint_pva.acceleration[joint.dof_index + 2U]},
            {joint_pva.acceleration[joint.dof_index + 3U],
             joint_pva.acceleration[joint.dof_index + 4U],
             joint_pva.acceleration[joint.dof_index + 5U]}
        };
        acceleration
