...
static BodyForce compute_body_com_force(
    ...
    const BodyForce& external_force)
{
    ...
    for (size_t index = 0; index < body_tree.get_num_bodies(); ++index)
    {
        ...
        // Accelerations and velocities in body-fixed coordinates
        const Vec3 acc_linear
            = rotate_mat3_transpose(rot, cartesian_motion.body[index].acceleration.linear);
        const Vec3 acc_angular
            = rotate_mat3_transpose(rot, cartesian_motion.body[index].acceleration.angular);
        const Vec3 vel_angular
            = rotate_mat3_transpose(rot, cartesian_motion.body[index].velocity.angular);
        ...
        // Compute torque
        const Vec3 torque_dyn = vector_add(
            inertia_multiply_vector(inertia, acc_angular),
            inertia_cross_multiply_vector(inertia, vel_angular));
        ...
        result.body[index].torque
            = vector_add(vector_subtract(torque_dyn, torque_ext), torque_from_force_com);
    }
    return result;
}
...
static JointSpace compute_body_joint_forces(
    const BodyTree& body_tree, const BodyCartesianPva& cartesian_motion, BodyForce& body_force)
{
    JointSpace result = {};
    for (size_t index = body_tree.get_num_bodies() - 1U; index > 0U; --index)
    {
        ...
        // body joint force
        const Vec3 force_child
            = quat_rotate_vector(parent_child_transform.rotation, body_force.body[index].force);
        body_force.body[parent_body_index].force
            = vector_add(body_force.body[parent_body_index].force, force_child);
        // body joint torque
        const Vec3 torque_child
            = quat_rotate_vector(parent_child_transform.rotation, body_force.body[index].torque);
        const Vec3 torque_from_force
            = vector_cross(parent_child_transform.translation, force_child);
        body_force.body[parent_body_index].torque = vector_add(
            body_force.body[parent_body_index].torque, vector_add(torque_child, torque_from_force));
    }
    return result;
}
